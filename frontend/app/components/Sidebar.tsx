...
const adminMenuItems: MenuItem[] = [
  ...
  { id: 'ai-assistant', label: 'Trợ lý HR', icon: <MessageSquareText className="size-5" />, badge: 'AI' },
...
const managerMenuItems: MenuItem[] = [
  ...
  { id: 'manager-tasks', label: 'Quản lý task', icon: <ClipboardList className="size-5" /> },
...
export function Sidebar({
  ...
  const menuItems = userRole === 'manager'
    ? managerMenuItems.map((item) =>
        item.id === 'employees'
          ? { ...item, badge: sessionDepartment || 'Team' }
          : item
      )
    : adminMenuItems;
  const roleInfo = userRole === 'manager'
    ? {
        ...
        subtitle: `Phòng ban ${sessionDepartment || 'được phân quyền'}`,
        summary: `Nhân viên ${sessionDepartment || 'trong team'}`,
  ...
  useEffect(() => {
    setSessionDepartment(window.localStorage.getItem('hrm_employee_department') || '');
  }, [userRole]);
            ...
            <div className="border-t border-white/10 p-3">
              <div className="rounded-xl border border-white/20 bg-white/10 px-3 py-2.5 backdrop-blur-md">
                <div className="flex items-center gap-2.5">
                  <div className="flex size-9 items-center justify-center rounded-full bg-gradient-to-br from-cyan-400 to-indigo-500 shadow-md">
                    <Users className="size-4" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold">{roleInfo.summary}</p>
                    <p className="truncate text-[11px] text-white/70">{roleInfo.summaryNote}</p>
                  </div>
                </div>
              </div>
            </div>
